...
RUN apt-get update \
  ...
  && apt-get update \
	&& DEBIAN_FRONTEND='noninteractive' apt-get install -y --allow-unauthenticated php7.2 php-memcache php-memcached php-redis php-xdebug wget \
	  curl composer php-gd php-gettext php-mbstring php-mcrypt php-mysql php-phpseclib php-sqlite3 mysql-client postgresql-client php-pgsql php-curl php-ldap php-zip\
